<script setup>
    ...
    const errors = ref([]);
    const updateProfile = () => {
        axios.put(`/api/profile/${user.value.id}`, {
            id : user.value.id,
    ...
    const handleChangePassword = () => {
        errors.value = '';
        axios.post(`/api/change-user-password/${user.value.id}`, changePasswordForm)
        .then((response) => {
            ...
            if(response.data.message){
                Swal.fire('Lỗi!', 'Bạn chưa nhập mật khẩu!', 'error');
            }
        })
        .catch((error) => {
            if (error.response && error.response.status === 422) {
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-body box-profile">
                            <div class="text-center">
                                ...
                                <img @click="openFileInput" class="profile-user-img img-circle" :src="user.avatar ? user.avatar : '/noimage.png'" alt="User profile picture">
                            </div>
